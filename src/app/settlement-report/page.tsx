...
export default function SettlementReport() {
  ...
  const downloadExcel = async () => {
    ...
    const SPECIAL_NOTES2: Record<string,string> = {"임순석":"5/15 CMS등록, 5/18 출금확인","서영민":"5/15 CMS등록 (임순석)","김이태":"5/15 CMS등록, 5/18 출금확인","장은경":"5/15 CMS등록, 5/18 출금확인","김건하":"1회 납부 후 정기결제 등록예정","김윤아":"카드 한도초과","박홍배":"임시 중단","최두식":"계좌문제 별도입금","백민엽":"3월 예약→계약 미전환","김나윤":"3월 예약→계약 미전환","신우진":"임시중단","윤권":"현장 딜레이","김정환":"현장문제 해소후","이연수":"4월 예약→계약 미전환","이정재":"매월 별도입금"};
    contacts.sort((a:any,b:any)=>{const na=parseInt((a.bunyanghoe_number||"").replace(/[^0-9]/g,""))||0;const nb=parseInt((b.bunyanghoe_number||"").replace(/[^0-9]/g,""))||0;return na-nb;}).forEach((c:any)=>{
      const num=parseInt((c.bunyanghoe_number||"").replace(/[^0-9]/g,""))||0;
        ...
        <>
          ...
          {(() => {
            ...
            const SPECIAL_NOTES: Record<string, string> = {
              "임순석": "5/15 효성CMS 등록, 5/18 출금 확인 예정",
              "서영민": "5/15 효성CMS 등록, 5/18 출금 확인 예정 (임순석)",
              "김이태": "5/15 효성CMS 등록, 5/18 출금 확인 예정",
              "장은경": "5/15 효성CMS 등록, 5/18 출금 확인 예정",
              "김건하": "1회 납부 후 정기결제 등록예정",
              "김윤아": "고객 사정, 카드 한도초과",
              "박홍배": "임시 중단요청",
              "최두식": "정기결제 등록, 계좌문제로 별도입금예정",
              "백민엽": "3월 예약완료→계약완료 미전환 (당월내 전환예정)",
              "김나윤": "3월 예약완료→계약완료 미전환 (당월내 전환예정)",
              "신우진": "광고효율 불만족 임시중단",
              "윤권": "예약 후 현장상황 문제로 계약전환 딜레이 (당월내 전환예정)",
              "김정환": "현장문제 해소 후 진행예정",
