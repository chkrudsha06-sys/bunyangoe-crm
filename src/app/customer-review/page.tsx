...
const gradeColor = (g: string) => g?.includes("마스터") ? "bg-amber-100 text-amber-700 border-amber-200" : g?.includes("챌린저") ? "bg-purple-100 text-purple-700 border-purple-200" : "bg-slate-100 text-slate-600 border-slate-200";
const admColor = (a: string) => a?.includes("적극") ? "bg-emerald-100 text-emerald-700 border-emerald-200" : a?.includes("가능") ? "bg-blue-100 text-blue-700 border-blue-200" : a?.includes("육성") ? "bg-orange-100 text-orange-700 border-orange-200" : "bg-red-100 text-red-600 border-red-200";
const scoreColor = (s: number) => s >= 80 ? "#16a34a" : s >= 65 ? "#3b82f6" : s >= 50 ? "#ea7c1e" : "#dc2626";
...
export default function CustomerReview() {
  ...
  const bronzes = reviews.filter(r => r.grade === "브론즈").length;
  const activeJoin = reviews.filter(r => r.admission?.includes("적극")).length;
  const possibleJoin = reviews.filter(r => r.admission?.includes("가능")).length;
  const nurture = reviews.filter(r => r.admission?.includes("육성")).length;
  const hold = reviews.filter(r => r.admission?.includes("보류")).length;
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>심사앱에서 저장된 고객 심사 데이터를 실시간 조회합니다</p>
        </div>
      ...
      <div className="grid grid-cols-7 gap-3">
        {[
          ...
          { label: "적극입회", count: activeJoin, color: "#16a34a", bg: "rgba(22,163,74,0.08)" },
          { label: "입회가능", count: possibleJoin, color: "#2563eb", bg: "rgba(37,99,235,0.08)" },
          { label: "보류", count: hold, color: "#dc2626", bg: "rgba(220,38,38,0.08)" },
        ].map(k => (
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select value={filterGrade} onChange={e => setFilterGrade(e.target.value)} className="text-xs px-3 py-2 rounded-xl outline-none" style={{ background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)" }}>
          ...
          <option value="마스터 후보">마스터</option>
          <option value="챌린저 후보">챌린저</option>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr style={{ background: "rgba(59,130,246,0.06)" }}>
                {["No","고객명","직급","소속회사","활동지역","총점","등급","입회판정","현장","조직","브랜딩","광고","심사일"].map(h => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={r.id} className="cursor-pointer transition-colors" style={{ borderBottom: "1px solid var(--border)" }}
                  ...
                  <td className="px-3 py-2.5 text-center font-bold" style={{ color: "var(--text)" }}>{r.field_score}/25</td>
                  <td className="px-3 py-2.5 text-center font-bold" style={{ color: "var(--text)" }}>{r.organization_score}/35</td>
                  <td className="px-3 py-2.5 text-center font-bold" style={{ color: "var(--text)" }}>{r.branding_score}/20</td>
                  <td className="px-3 py-2.5 text-center font-bold" style={{ color: "var(--text)" }}>{r.ad_score}/20</td>
          ...
          <div className="relative w-full max-w-md h-full overflow-y-auto shadow-2xl" style={{ background: "var(--bg)" }} onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="text-center py-5 rounded-2xl" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                <p className="text-4xl font-black" style={{ color: scoreColor(selected.total_score) }}>{selected.total_score}점</p>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: "현장 운영력", score: selected.field_score, max: 25, color: "#3b82f6" },
                  { label: "조직 운영력", score: selected.organization_score, max: 35, color: "#8b5cf6" },
                  { label: "브랜딩", score: selected.branding_score, max: 20, color: "#ea7c1e" },
                  { label: "광고 집행력", score: selected.ad_score, max: 20, color: "#16a34a" },
                ].map(s => (
