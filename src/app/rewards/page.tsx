...
export default function RewardsPage() {
  ...
  const handlePayment = async (row: RewardRow) => {
    ...
    await supabase.from("rewards").update({ is_paid: true, paid_date: today, remaining_reward: 0 }).eq("id", row.id);
    fetchRewards();
  };
  ...
  const handleMileageUse = async (row: RewardRow) => {
    ...
    const today = new Date().toISOString().split("T")[0];
    await supabase.from("rewards").update({ mileage_used: row.mileage_used + Number(amount), mileage_use_date: today }).eq("id", row.id);
    fetchRewards();
  };
    ...
    <div className="flex flex-col h-full bg-[#F1F5F9]">
      <div className="bg-white border-b border-slate-200 px-6 py-4 sticky top-0 z-10">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setShowAddModal(true)} className="flex items-center gap-2 px-4 py-2 bg-[#1E3A8A] text-white text-sm font-semibold rounded-lg hover:bg-[#1e40af] transition-colors shadow-sm">
            <Plus size={14} />리워드 추가
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 mt-3">
          <select value={filterQuarter} onChange={(e) => setFilterQuarter(e.target.value)} className="text-sm px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-700">
            <option value="">전체 분기</option>
            {quarters.map((q) => <option key={q} value={q}>{getQuarterLabel(q)}</option>)}
          </select>
          <select value={filterPaid} onChange={(e) => setFilterPaid(e.target.value)} className="text-sm px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-700">
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  {["넘버링", "고객명", "분기", "호갱노노리워드(5%)", "LMS리워드(15%)", "하이타겟리워드(5%)", "하이타겟마일리지(10%)", "누적리워드", "지급예정월", "소득세(3.3%)", "잔여리워드", "마일리지사용", "지급여부", "액션"].map((h) => (
                ...
                {rewards.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      <td className="px-3 py-2.5"><span className="text-xs font-mono bg-slate-100 px-1.5 py-0.5 rounded text-slate-600">{row.member_number || "-"}</span></td>
                      <td className="px-3 py-2.5 font-semibold text-slate-800 text-sm">{row.member_name}</td>
                      <td className="px-3 py-2.5"><span className="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full border border-blue-100">{row.quarter}</span></td>
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex flex-col">
                          ...
                          {row.mileage_used > 0 && <span className="text-xs text-red-500">-{formatWon(row.mileage_used)} 사용</span>}
                        </div>
                      </td>
                      <td className="px-3 py-2.5"><span className="font-bold text-[#C9A84C]">{formatWon(row.accumulated_reward)}</span></td>
                      <td className="px-3 py-2.5"><span className="text-xs font-medium text-slate-600">{row.payment_due_month ? `${row.payment_due_month} 15일` : calcPaymentDueMonth(row.quarter) ? `${calcPaymentDueMonth(row.quarter)} 15일` : "-"}</span></td>
                      <td className="px-3 py-2.5"><span className="text-xs text-red-500">-{formatWon(row.income_tax || Math.floor(row.accumulated_reward * 0.033))}</span></td>
                      <td className="px-3 py-2.5"><span className={`font-semibold text-sm ${row.is_paid ? "text-slate-400 line-through" : "text-emerald-600"}`}>{row.is_paid ? "지급완료" : formatWon(row.remaining_reward || (row.accumulated_reward - Math.floor(row.accumulated_reward * 0.033)))}</span></td>
                      <td className="px-3 py-2.5">
                        {mileageRemaining > 0 ? (
                          <button onClick={() => handleMileageUse(row)} className="text-xs text-blue-600 hover:text-blue-800 underline">{formatWon(mileageRemaining)} 잔여</button>
                        ) : <span className="text-xs text-slate-400">-</span>}
                      </td>
                      <td className="px-3 py-2.5">
                        {row.is_paid ? (
                          <div className="flex items-center gap-1"><CheckCircle size={14} className="text-emerald-500" /><span className="text-xs text-emerald-600 font-medium">{row.paid_date ? new Date(row.paid_date).toLocaleDateString("ko-KR", { month: "short", day: "numeric" }) : "완료"}</span></div>
                        ) : (
                          <div className="flex items-center gap-1"><AlertCircle size={14} className="text-amber-500" /><span className="text-xs text-amber-600 font-medium">미지급</span></div>
                        )}
                      </td>
                      <td className="px-3 py-2.5">
                        {!row.is_paid && (
                          <button onClick={() => handlePayment(row)} className="text-xs px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-lg border border-emerald-200 hover:bg-emerald-100 font-medium">지급처리</button>
                        )}
                      </td>
                    </tr>
                  );
