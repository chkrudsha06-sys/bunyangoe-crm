...
import { Camera, FileText, Send, CheckCircle, Upload, X, Save } from "lucide-react";
...
function isImageFile(file: UploadedFile): boolean {
  return /\.(jpg|jpeg|png|gif|webp|heic|heif)$/i.test(file.name || "") || file.url?.startsWith("data:image");
}
...
export default function ContentManagePage() {
  ...
  const fetchData = async () => {
    ...
    const { data: extContacts } = await supabase
      .from("contacts")
      .select("id,name,title,bunyanghoe_number,meeting_result,assigned_to")
    ...
    // 파일까지 함께 로드합니다.
    // 기존에는 files를 제외하고 files: []로 초기화해서 업로드 후 목록이 사라져 보일 수 있었습니다.
    const { data: cs } = await supabase
      .from("content_statuses")
      .select(
        "id,contact_id,photo_received,info_received,tf2_delivered,pr_completed,production_impossible,impossible_reason,files,pr_name,pr_gender,pr_birth_date,pr_title_position,pr_age,pr_height,pr_body_type,pr_activity_region,pr_company,pr_site_history_1,pr_site_history_2,pr_site_history_3,pr_site_history_4,pr_site_history_5,pr_intro,pr_request,pr_video_copy_1,pr_video_performance,pr_video_copy_2,pr_site_info,pr_photo_desc,pr_feed_text,pr_career,pr_years,pr_years_base_year,pr_output_server,pr_output_url,updated_at"
      );
    ...
    (cs || []).forEach((s: any) => {
      map[s.contact_id] = {
        ...
        files: normalizeFiles(s.files),
      };
    });

    setStatuses(map);
    setLoadedFiles(new Set(Object.keys(map).map(Number)));
    setLoading(false);
  };
  ...
  const handleRegister = async () => {
    ...
    const { data: contact, error } = await supabase
      ...
      .insert({
        ...
        assigned_to: regForm.assigned_to.trim() || null,
  ...
  const expandMember = async (contactId: number) => {
    ...
    if (data) {
      ...
      setStatuses(prev => ({
        ...prev,
        [contactId]: {
          ...(prev[contactId] || { ...EMPTY_STATUS, contact_id: contactId }),
          files,
        },
      }));
  ...
  const handleFileUpload = async (contactId: number, fileList: FileList | File[]) => {
    ...
    try {
      ...
      // DB의 최신 files를 다시 읽은 뒤 병합합니다.
      // 이렇게 해야 이미 저장되어 있던 파일 목록이 덮어써지거나 사라지는 문제를 막을 수 있습니다.
      ...
      if (s.id) {
        ...
        currentFiles = normalizeFiles(latestStatus?.files);
      } else {
        currentFiles = normalizeFiles(s.files);
      }
  ...
  const deleteFile = async (contactId: number, fileIndex: number, fileName: string) => {
    const s = getStatus(contactId);
    const currentFiles = normalizeFiles(s.files);
  ...
  const saveInfo = async (contactId: number) => {
    ...
    const payload = {
      contact_id: contactId,
      pr_name: s.pr_name,
      pr_gender: s.pr_gender,
      pr_birth_date: s.pr_birth_date,
      pr_title_position: s.pr_title_position,
      pr_age: s.pr_age,
      pr_height: s.pr_height,
      pr_body_type: s.pr_body_type,
      pr_activity_region: s.pr_activity_region,
      pr_company: s.pr_company,
      pr_site_history_1: s.pr_site_history_1,
      pr_site_history_2: s.pr_site_history_2,
      pr_site_history_3: s.pr_site_history_3,
      pr_site_history_4: s.pr_site_history_4,
      pr_site_history_5: s.pr_site_history_5,
      pr_intro: s.pr_intro,
      pr_request: s.pr_request,
      pr_video_copy_1: s.pr_video_copy_1,
      pr_video_performance: s.pr_video_performance,
      pr_video_copy_2: s.pr_video_copy_2,
      pr_site_info: s.pr_site_info,
      pr_photo_desc: s.pr_photo_desc,
      pr_feed_text: s.pr_feed_text,
      pr_career: s.pr_career,
      pr_years: s.pr_years,
      pr_years_base_year: s.pr_years_base_year || new Date().getFullYear(),
      pr_output_server: s.pr_output_server,
      pr_output_url: s.pr_output_url,
      info_received: true,
      updated_at: new Date().toISOString(),
    };
    ...
    <div className="flex flex-col h-full" style={{ background: "var(--bg)" }}>
      ...
      <div className="px-6 py-4 flex-shrink-0" style={{ borderBottom: "1px solid var(--border)" }}>
        ...
        <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
          PR패키지 제작을 위한 자료 수취 및 TF2팀 전달 관리
        </p>
      </div>
      ...
      <div className="px-6 py-3 flex gap-3 flex-shrink-0 flex-wrap">
        {[
          ...
          return (
            <button
              key={c.key}
              onClick={() => setCardFilter(isActive ? "" : c.key)}
            ...
            >
              ...
              <p className="text-[10px] font-semibold" style={{ color: isActive ? c.color : "var(--text-muted)" }}>
                {c.label}
              </p>
            </button>
          );
            ...
            return (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="rounded-xl p-4" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                  <h4 className="text-xs font-bold mb-3" style={{ color: "var(--text-muted)" }}>
                    성별 분포
                  </h4>
                    ...
                    {Object.entries(genderData)
                      ...
                      .map(([k, v]) => (
                        <div key={k} className="text-center">
                          <p className="text-2xl font-black" style={{ color: genderColors[k] }}>
                            {v}
                          </p>
                      ...
                      {ageKeys.map(k => {
                        ...
                        return (
                          <div key={k} className="flex items-center gap-2">
                            <span className="text-xs font-bold w-10 text-right" style={{ color: "var(--text)" }}>
                              {k}
                            </span>
      ...
      <div className="px-6 py-3 flex items-center gap-2 flex-shrink-0 flex-wrap" style={{ borderBottom: "1px solid var(--border)" }}>
        ...
        >
          ...
          {Array.from(new Set(members.map(m => m.assigned_to).filter(Boolean)))
            .sort()
            .map(a => (
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="overflow-y-auto pb-4" style={{ width: 350, flexShrink: 0, borderRight: "1px solid var(--border)" }}>
          <div className="px-3 py-2" style={{ borderBottom: "1px solid var(--border)" }}>
            <p className="text-xs font-bold" style={{ color: "var(--text)" }}>
              🏆 분양회 회원 <span className="font-normal" style={{ color: "var(--text-muted)" }}>({members.length}명)</span>
            </p>
          </div>
          ...
          {(() => {
            ...
            if (cardFilter && cardFilter !== "all") {
              filtered = filtered.filter(m => {
                ...
                switch (cardFilter) {
                  case "photo":
                    return s.photo_received;
                  case "info":
                    return s.info_received;
                  case "tf2":
                    return s.tf2_delivered;
                  case "pr":
                    return s.pr_completed;
                  case "impossible":
                    return s.production_impossible;
                  default:
                    return true;
                }
              });
                    ...
                    return (
                      ...
                      >
                        <span
                          ...
                          style={{ background: "rgba(59,130,246,0.1)", color: "#3b82f6", minWidth: 36, textAlign: "center" }}
                        >
        ...
        <div className="flex-1 overflow-y-auto pb-4">
          <div className="px-3 py-2 flex items-center justify-between" style={{ borderBottom: "1px solid var(--border)" }}>
            <p className="text-xs font-bold" style={{ color: "var(--text)" }}>
              🎬 외부 컨텐츠 고객 <span className="font-normal" style={{ color: "var(--text-muted)" }}>({extMembers.length}명)</span>
            </p>
            <button
          ...
          {showRegForm && (
            <div className="p-3 space-y-2" style={{ borderBottom: "1px solid var(--border)" }}>
              <div className="grid grid-cols-2 gap-2">
                ...
                <input
                  placeholder="대협팀 담당자"
                  value={regForm.assigned_to}
                  onChange={e => setRegForm(p => ({ ...p, assigned_to: e.target.value }))}
                  className="px-2.5 py-1.5 text-xs rounded-lg outline-none"
                  style={{ background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)" }}
                />
                <input
                  placeholder="컨텐츠제작항목"
                  value={regForm.content_item}
                  onChange={e => setRegForm(p => ({ ...p, content_item: e.target.value }))}
                  className="px-2.5 py-1.5 text-xs rounded-lg outline-none"
                  style={{ background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)" }}
                />
              </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={regSaving || !regForm.name.trim()}
                  className="flex-1 py-1.5 text-xs font-bold text-white rounded-lg disabled:opacity-50"
              ...
              {extMembers.map(m => {
                ...
                return (
                  <div
                    ...
                    onClick={() => expandMember(m.id)}
                    style={{
                      background: expandedId === m.id ? "rgba(59,130,246,0.08)" : "var(--surface)",
                      border: `1px solid ${expandedId === m.id ? "rgba(59,130,246,0.3)" : "var(--border)"}`,
                    }}
                  >
      ...
      {expandedId &&
        (() => {
          ...
          return (
            <div className="fixed inset-0 z-50 flex justify-end" style={{ background: "rgba(0,0,0,0.4)" }} onClick={() => setExpandedId(null)}>
              <div
                className="h-full overflow-y-auto shadow-2xl"
                style={{ width: "75%", maxWidth: 1200, background: "var(--modal-bg)", borderLeft: "1px solid var(--border)" }}
              ...
              >
                <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-3" style={{ background: "var(--modal-bg)", borderBottom: "1px solid var(--border)" }}>
                  <div className="flex items-center gap-3">
                    {selectedMember.bunyanghoe_number && (
                    ...
                    {selectedMember.assigned_to && (
                      <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(139,92,246,0.1)", color: "#8b5cf6" }}>
                        {selectedMember.assigned_to}
                      </span>
                    )}
                  </div>
                  <button onClick={() => setExpandedId(null)} className="p-2 rounded-lg text-lg" style={{ color: "var(--text-muted)" }}>
                    ✕
                  </button>
                </div>
                  ...
                  <div className="flex gap-4" style={{ alignItems: "flex-start" }}>
                    ...
                    <div style={{ width: "40%", flexShrink: 0 }} className="space-y-3">
                      <div className="flex items-center gap-3 flex-wrap rounded-xl p-3" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                        {[
                          ...
                          <label key={item.field} className="flex items-center gap-1.5 cursor-pointer text-xs" style={{ color: "var(--text)" }}>
                            <input
                              type="checkbox"
                              checked={(s as any)[item.field] || false}
                              onChange={() => toggleCheckbox(m.id, item.field)}
                              className="w-3.5 h-3.5 rounded accent-blue-500"
                            />
                            <item.icon
                              size={12}
                              style={{ color: (s as any)[item.field] ? (item.field === "production_impossible" ? "#ef4444" : "#10b981") : "var(--text-muted)" }}
                            />
                            {item.label}
                          </label>
                        ))}
                      ...
                      <div className="rounded-xl p-4" style={{ background: "var(--bg)", border: "1px solid var(--border)" }}>
                        ...
                        {files.length > 0 && (
                          <div className="space-y-1.5 mb-3">
                            {files.map((file, fi) => (
                              <div key={`${file.path || file.url || file.name}-${fi}`} className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                                {isImageFile(file) ? (
                                ...
                                <button
                                  onClick={() => deleteFile(m.id, fi, file.name)}
                                  className="px-2 py-1 rounded-lg text-[10px] font-semibold"
                        ...
                        <div
                          ...
                          onDrop={e => {
                            e.preventDefault();
                            e.currentTarget.style.borderColor = "var(--border)";
                            if (e.dataTransfer.files.length > 0) handleFileUpload(m.id, e.dataTransfer.files);
                          }}
                          onClick={() => {
                        ...
                        >
                          {uploading === m.id ? (
                            ...
                            <>
                              <Upload size={16} />
                              <span className="text-xs">클릭 또는 드래그 업로드</span>
                            </>
                          )}
                        </div>
                        <p className="text-[10px] text-center mt-1.5" style={{ color: "var(--text-subtle)" }}>
                          최대 업로드 용량: 파일당 50MB
                        </p>
                      </div>
                      ...
                      <div className="rounded-xl p-4 space-y-2.5" style={{ background: "var(--bg)", border: "1px solid var(--border)" }}>
                        <h4 className="text-sm font-bold flex items-center gap-1.5" style={{ color: "var(--text)" }}>
                          📁 산출물 주소
                        </h4>
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {s.pr_output_server && (
                              <button
                                onClick={() => {
                                  navigator.clipboard.writeText(s.pr_output_server);
                                  showToast("경로 복사됨");
                                }}
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1">
                            {s.pr_output_url ? (
                              <a
                                href={s.pr_output_url.startsWith("http") ? s.pr_output_url : `https://${s.pr_output_url}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="flex-1 px-2.5 py-1.5 text-xs rounded-lg truncate block"
                                style={{ background: "var(--surface)", border: "1px solid rgba(59,130,246,0.3)", color: "#3b82f6", textDecoration: "underline" }}
                              >
                                {s.pr_output_url}
                              </a>
                            ) : (
                            ...
                            {s.pr_output_url && (
                              <button
                                onClick={() => {
                                  navigator.clipboard.writeText(s.pr_output_url);
                                  showToast("URL 복사됨");
                                }}
                                className="px-2 py-1.5 rounded-lg text-xs flex-shrink-0"
                                style={{ background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text-muted)" }}
                        ...
                        <details className="text-xs">
                          <summary className="cursor-pointer font-semibold" style={{ color: "#3b82f6" }}>
                            주소 편집
                          </summary>
                          <div className="mt-2 space-y-2">
                            <div>
                              <label className="text-[10px] font-semibold block mb-0.5" style={{ color: "var(--text-muted)" }}>
                                서버/컴퓨터 경로
                              </label>
                              <input
                                className="w-full px-2.5 py-1.5 text-xs rounded-lg outline-none"
                                value={s.pr_output_server || ""}
                                onChange={e => updateField(m.id, "pr_output_server", e.target.value)}
                                placeholder="\\\\server\\path\\folder"
                                style={{ background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)" }}
                              />
                            </div>
                            <div>
                              ...
                              <input
                                className="w-full px-2.5 py-1.5 text-xs rounded-lg outline-none"
                                value={s.pr_output_url || ""}
                                onChange={e => updateField(m.id, "pr_output_url", e.target.value)}
                        ...
                        {s.production_impossible ? (
                          <>
                            ...
                            <button
                              onClick={async () => {
                                ...
                                const payload = {
                                  contact_id: m.id,
                                  impossible_reason: s.impossible_reason || "",
                                ...
                                if (s.id) {
                                  ...
                                  const { data } = await supabase.from("content_statuses").insert(payload).select().single();
                                  if (data) updateField(m.id, "id", data.id);
                                }
                          ...
                          <>
                            ...
                            <div className="rounded-xl p-3 mb-3" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                              <h5 className="text-xs font-bold mb-2 pb-1.5" style={{ color: "#3b82f6", borderBottom: "1px solid var(--border)" }}>
                                📋 고객기본정보
                              </h5>
                              ...
                              <div className="grid grid-cols-3 gap-x-3 gap-y-2">
                                {[
                                  ...
                                  {
                                    key: "pr_years",
                                    label: `연차${s.pr_years_base_year ? ` (${calcYears(s.pr_years_base_year, s.pr_years)})` : ""}`,
                                    placeholder: "7년차",
                                  },
                                ].map(f => (
                                  <div key={f.key}>
                              ...
                              <div className="mt-2">
                                ...
                                <textarea
                                  className="w-full px-2.5 py-2 text-sm rounded-lg outline-none resize-none"
                                  value={(s as any).pr_request || ""}
                            ...
                            <div className="grid grid-cols-3 gap-3 mb-3">
                              <div className="rounded-xl p-3" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                                <h5 className="text-xs font-bold mb-2 pb-1.5" style={{ color: "#10b981", borderBottom: "1px solid var(--border)" }}>
                                  🏗️ 대표현장이력
                                </h5>
                                <div className="space-y-1.5">
                                  {[
                                    ...
                                    <input
                                      ...
                                      value={(s as any)[`pr_site_history_${n}`] || ""}
                              ...
                              <div className="rounded-xl p-3" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                                <h5 className="text-xs font-bold mb-2 pb-1.5" style={{ color: "#8b5cf6", borderBottom: "1px solid var(--border)" }}>
                                  🎬 ①영상카피
                                </h5>
                                <label className="text-xs font-semibold block mb-1" style={{ color: "var(--text-muted)" }}>
                                  문구기재
                                ...
                                <input
                                  className="w-full px-2.5 py-2 text-sm rounded-lg outline-none"
                                  value={(s as any).pr_video_copy_1 || ""}
                              ...
                              <div className="rounded-xl p-3" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                                ...
                                <div className="space-y-2">
                                  <div>
                                    ...
                                    <input
                                      className="w-full px-2.5 py-2 text-sm rounded-lg outline-none"
                                      value={(s as any).pr_video_performance || ""}
                                  ...
                                  <div>
                                    ...
                                    <input
                                      className="w-full px-2.5 py-2 text-sm rounded-lg outline-none"
                                      value={(s as any).pr_video_copy_2 || ""}
      ...
      <input
        ...
        onChange={e => {
          ...
          if (files && files.length > 0 && targetId) {
            handleFileUpload(targetId, files);
          }
