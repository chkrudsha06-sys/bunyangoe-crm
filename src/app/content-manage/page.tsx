...
import { useState, useEffect, useRef } from "react";import { Camera, FileText, Send, CheckCircle, Upload, Download, X, ChevronDown, ChevronUp, Save } from "lucide-react";import { supabase } from "@/lib/supabase";

interface VipMember {id: number;name: string;title: string | null;bunyanghoe_number: string | null;meeting_result: string | null;assigned_to: string | null;}

interface UploadedFile {name: string;url: string;size?: number;path?: string;uploaded_at: string;}

interface ContentStatus {id?: number;contact_id: number;photo_received: boolean;info_received: boolean;tf2_delivered: boolean;pr_completed: boolean;production_impossible: boolean;impossible_reason: string;files: UploadedFile[];pr_name: string;pr_gender: string;pr_birth_date: string;pr_title_position: string;pr_age: string;pr_height: string;pr_body_type: string;pr_activity_region: string;pr_company: string;pr_site_history_1: string;pr_site_history_2: string;pr_site_history_3: string;pr_site_history_4: string;pr_site_history_5: string;pr_intro: string;pr_request: string;pr_video_copy_1: string;pr_video_performance: string;pr_video_copy_2: string;pr_site_info: string;pr_photo_desc: string;pr_feed_text: string;pr_career: string;pr_years: string;pr_years_base_year: number;pr_output_server: string;pr_output_url: string;updated_at: string | null;}

const EMPTY_STATUS: Omit<ContentStatus, "contact_id"> = {photo_received: false, info_received: false, tf2_delivered: false, pr_completed: false,production_impossible: false, impossible_reason: "",files: [],pr_name: "", pr_gender: "", pr_birth_date: "", pr_title_position: "", pr_age: "", pr_height: "", pr_body_type: "",pr_activity_region: "", pr_company: "",pr_site_history_1: "", pr_site_history_2: "", pr_site_history_3: "", pr_site_history_4: "", pr_site_history_5: "",pr_intro: "", pr_request: "", pr_video_copy_1: "", pr_video_performance: "", pr_video_copy_2: "",pr_site_info: "", pr_photo_desc: "", pr_feed_text: "", pr_career: "", pr_years: "", pr_years_base_year: 0,pr_output_server: "", pr_output_url: "", updated_at: null,};

// 연차 자동계산function calcYears(baseYear: number, storedYears: string): string {if (!baseYear || !storedYears) return storedYears || "";const num = parseInt(storedYears.replace(/[^0-9]/g, ""));if (isNaN(num)) return storedYears;const currentYear = new Date().getFullYear();const diff = currentYear - baseYear;return ${num + diff}년차;}

// Supabase Storage에 파일 업로드 (원본 품질 유지)async function uploadToStorage(file: File, contactId: number): Promise<{ name: string; url: string; size: number; path: string }> {const timestamp = Date.now();const safeName = file.name.replace(/[^a-zA-Z0-9가-힣.-]/g, "");const path = ${contactId}/${timestamp}_${safeName};

const { error } = await supabase.storage.from("content-files").upload(path, file, {cacheControl: "3600",upsert: false,});if (error) throw new Error(업로드 실패: ${error.message});

const { data: urlData } = supabase.storage.from("content-files").getPublicUrl(path);return { name: file.name, url: urlData.publicUrl, size: file.size, path };}

export default function ContentManagePage() {const [members, setMembers] = useState<VipMember[]>([]);const [extMembers, setExtMembers] = useState<VipMember[]>([]);const [statuses, setStatuses] = useState<Record<number, ContentStatus>>({});const [loading, setLoading] = useState(true);const [expandedId, setExpandedId] = useState<number | null>(null);const [loadedFiles, setLoadedFiles] = useState<Set>(new Set());const [showRegForm, setShowRegForm] = useState(false);const [regForm, setRegForm] = useState({ name: "", title: "", assigned_to: "", content_item: "" });const [regSaving, setRegSaving] = useState(false);

// 새 고객 등록const handleRegister = async () => {if (!regForm.name.trim()) return;setRegSaving(true);const { data: contact, error } = await supabase.from("contacts").insert({name: regForm.name.trim(), title: regForm.title.trim() || null,assigned_to: regForm.assigned_to.trim() || null, meeting_result: "컨텐츠등록",memo: regForm.content_item.trim() || null,}).select().single();if (error || !contact) { alert("등록 실패: " + (error?.message || "")); setRegSaving(false); return; }await supabase.from("content_statuses").insert({ contact_id: contact.id });setRegForm({ name: "", title: "", assigned_to: "", content_item: "" });setShowRegForm(false); setRegSaving(false);fetchData();};

// 회원 펼칠 때 파일 데이터 로드 (lazy loading)const expandMember = async (contactId: number) => {if (expandedId === contactId) { setExpandedId(null); return; }setExpandedId(contactId);
...
};const [saving, setSaving] = useState<number | null>(null);const [uploading, setUploading] = useState<number | null>(null);const [toast, setToast] = useState("");const [searchQ, setSearchQ] = useState("");const [filterAssigned, setFilterAssigned] = useState("");const [filterStatus, setFilterStatus] = useState("");const [filterGender, setFilterGender] = useState("");const [filterAge, setFilterAge] = useState("");const [cardFilter, setCardFilter] = useState("");const [showStats, setShowStats] = useState(true);const fileRef = useRef(null);const [uploadTarget, setUploadTarget] = useState<number | null>(null);

useEffect(() => { fetchData(); }, []);

const showToast = (msg: string) => { setToast(msg); setTimeout(() => setToast(""), 2500); };

const fetchData = async () => {setLoading(true);// VIP 입회자 (계약완료/예약완료)const { data: contacts } = await supabase.from("contacts").select("id,name,title,bunyanghoe_number,meeting_result,assigned_to").in("meeting_result", ["계약완료", "예약완료"]);
...
const getStatus = (contactId: number): ContentStatus => {return statuses[contactId] || { ...EMPTY_STATUS, contact_id: contactId };};

const updateField = (contactId: number, field: string, value: any) => {setStatuses(prev => {const current = prev[contactId] || { ...EMPTY_STATUS, contact_id: contactId };
...
const toggleCheckbox = async (contactId: number, field: string) => {const s = getStatus(contactId);const newVal = !(s as any)[field];
...
const handleFileUpload = async (contactId: number, fileList: FileList | File[]) => {setUploading(contactId);const s = getStatus(contactId);const existingFiles = [...(s.files || [])];
...
const deleteFile = async (contactId: number, fileIndex: number, fileName: string) => {const s = getStatus(contactId);if (!s.files || s.files.length === 0) { showToast("삭제할 파일이 없습니다"); return; }
...
const downloadFile = (file: UploadedFile) => {// Storage URL → 새 탭에서 원본 다운로드if (file.url.startsWith("http")) {window.open(file.url, "_blank");return;}// 기존 base64 호환const a = document.createElement("a");a.href = file.url;a.download = file.name;document.body.appendChild(a);a.click();document.body.removeChild(a);};

const fmtSize = (bytes?: number) => {if (!bytes) return "";if (bytes < 1024) return ${bytes}B;if (bytes < 1024 * 1024) return ${(bytes / 1024).toFixed(0)}KB;return ${(bytes / (1024 * 1024)).toFixed(1)}MB;};

const saveInfo = async (contactId: number) => {setSaving(contactId);const s = getStatus(contactId);const payload = {contact_id: contactId,pr_name: s.pr_name, pr_gender: s.pr_gender, pr_birth_date: s.pr_birth_date,pr_title_position: s.pr_title_position, pr_age: s.pr_age,pr_height: s.pr_height, pr_body_type: s.pr_body_type,pr_activity_region: s.pr_activity_region, pr_company: s.pr_company,pr_site_history_1: s.pr_site_history_1, pr_site_history_2: s.pr_site_history_2,pr_site_history_3: s.pr_site_history_3, pr_site_history_4: s.pr_site_history_4,pr_site_history_5: s.pr_site_history_5,pr_intro: s.pr_intro,pr_request: s.pr_request,pr_video_copy_1: s.pr_video_copy_1, pr_video_performance: s.pr_video_performance,pr_video_copy_2: s.pr_video_copy_2,pr_site_info: s.pr_site_info, pr_photo_desc: s.pr_photo_desc,pr_feed_text: s.pr_feed_text, pr_career: s.pr_career, pr_years: s.pr_years,pr_years_base_year: s.pr_years_base_year || new Date().getFullYear(),pr_output_server: s.pr_output_server, pr_output_url: s.pr_output_url,info_received: true,updated_at: new Date().toISOString(),};if (s.id) {await supabase.from("content_statuses").update(payload).eq("id", s.id);} else {const { data } = await supabase.from("content_statuses").insert(payload).select().single();if (data) updateField(contactId, "id", data.id);}updateField(contactId, "info_received", true);setSaving(null);showToast("기본정보 저장 완료");};

const StatusBadge = ({ done, label }: { done: boolean; label: string }) => (<span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-semibold"style={{background: done ? "rgba(16,185,129,0.1)" : "rgba(148,163,184,0.1)",color: done ? "#10b981" : "#94a3b8",border: 1px solid ${done ? "rgba(16,185,129,0.2)" : "rgba(148,163,184,0.15)"},}}>{done ?  : <span style={{ width: 11, height: 11, borderRadius: "50%", border: "1.5px solid currentColor", display: "inline-block" }} />}{label});

const inp = "w-full px-3 py-2 text-sm rounded-xl outline-none focus focus";const lbl = "block text-xs font-semibold mb-1";

return (<div className="flex flex-col h-full" style={{ background: "var(--bg)" }}>{/* 헤더 */}<div className="px-6 py-4 flex-shrink-0" style={{ borderBottom: "1px solid var(--border)" }}><h1 className="text-lg font-bold" style={{ color: "var(--text)" }}>🎬 분양회 회원 컨텐츠 관리<p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>PR패키지 제작을 위한 자료 수취 및 TF2팀 전달 관리
