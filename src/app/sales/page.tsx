...
export default function SalesPage() {
  ...
  const channels     = isBunyanghoe ? CHANNELS_BUNYANGHOE : CHANNELS_WANPAN;
                ...
                {executions.map(e=>(
                  <tr key={e.id} className="border-b border-slate-100 hover:bg-slate-50">
                    <td className="px-3 py-2.5">
                      <span className={`text-xs px-1.5 py-0.5 rounded border ${e.contract_route==="분양회"?"bg-amber-50 text-amber-700 border-amber-100":e.contract_route==="완판트럭"?"bg-emerald-50 text-emerald-700 border-emerald-100":"bg-slate-50 text-slate-500 border-slate-100"}`}>
                        {e.contract_route||"-"}
                      </span>
                    </td>
